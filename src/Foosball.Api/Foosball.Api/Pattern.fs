...
module Pattern =
    ...
    module Goal =
        ...
        let rec (|DurationBetweenGoals|_|) =
            ...
            | x :: DurationBetweenGoals((duration,y) :: tail) -> 
                Some((x.gametime - y.gametime,x) :: (duration,y) :: tail)
            | _ -> None
        
        let (|GoalWithinSeconds|_|) seconds list =
            match list with
            | IsGoal _ :: _ & Goals(DurationBetweenGoals((duration,x) :: tail)) when duration.TotalSeconds 
                                                                                     <= seconds -> 
                Some((duration,x) :: tail)
            | _ -> None
    
    let (|AllPlayersRegistered|_|) input =
        match input |> List.rev with
        | [Configure _;Register _;Register _;Register _;Register _] -> Some()
        | Configure _ :: Register _ :: Register _ :: Register _ :: Register _ :: _ -> 
            Some()
        | _ -> None
    ...
    let (|RegisterAllPlayers|_|)(state,event) =
        ...
        | MissingOnePlayer(wd,wa,bd),Register ba -> 
            ...
            start :: RegisterTeam black :: RegisterTeam white :: event :: state 
            |> Some
        | _ -> None
    ...
    let (|RegisterWhiteDefense|_|) input =
        match input with
        | [Configure _] -> 
            Some([Player.zero;Player.zero;Player.zero;Player.zero])
        | _ -> None
    
    let (|RegisterWhiteAttack|_|) input =
        match input with
        | [Register a;Configure _] -> 
            Some([a;Player.zero;Player.zero;Player.zero])
        | _ -> None
    
    let (|RegisterBlackDefense|_|) =
        function 
        | [Register b;Register a;Configure _] -> 
            Some([a;b;Player.zero;Player.zero])
        | _ -> None
    
    let (|RegisterBlackAttack|_|) input =
        match input with
        | [Register c;Register b;Register a;Configure _] -> 
            Some([a;b;c;Player.zero])
        | _ -> None
    
    let (|RegisteredPlayers|_|) input =
        match input with
        | [Register d;Register c;Register b;Register a;Configure _] -> 
            Some("READY",[a;b;c;d],"Throw in the ball to start the game.")
        | RegisterBlackAttack(a) -> 
            Some("Configure",a,"Register player for Black Attack position")
        | RegisterBlackDefense(a) -> 
            Some("Configure",a,"Register player for Black Defense position")
        | RegisterWhiteAttack(a) -> 
            Some("Configure",a,"Register player for White Attack position")
        | RegisterWhiteDefense(a) -> 
            Some("Configure",a,"Register player for White Defense position")
        | _ -> None
