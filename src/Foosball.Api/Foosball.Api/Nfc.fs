...
module CardReader = 
    ...
    let streamMapper insert remove = 
        ...
        |> Observable.scan  (fun acc item -> item acc) NoCard
    ...
    let connect (monitor : ISCardMonitor) (context : ISCardContext) (readerId : string) = 
        ...
        let insertAction _ _ = extractId (readerId, readerCreator) |> Inserted
        let insert = monitor.CardInserted |> Observable.map insertAction
        let remove = monitor.CardRemoved |> Observable.map (fun _ _ ->Removed)
    ...
    let start (monitor : ISCardMonitor) (context : ISCardContext) (readerId : string) = 
        async { 
            ...
            while stdin.ReadLine() = "exit" |> not do                
    ...
    //    let evI = new Event<_>()
    //    let evR = new Event<_>()
    //    let insert = evI.Publish |> Observable.map (fun a _ ->  Inserted a)
    //    let remove = evR.Publish |> Observable.map (fun _ b -> Removed b)
